...
const ModalMahasiswa = ({ isOpen, onClose, onSubmit, selectedData }) => {
  const [form, setForm] = useState({ nim: "", name: "", max_sks: "" });

  useEffect(() => {
    if (selectedData) {
      setForm(selectedData);
    } else {
      setForm({ nim: "", name: "", max_sks: "" });
    }
  }, [selectedData, isOpen]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 transition-all duration-300">
      <div className="bg-white w-full max-w-md p-6 rounded-2xl shadow-lg transform transition-all duration-300">
        <h2 className="text-2xl font-semibold text-center mb-6 text-gray-800">
          {selectedData ? "Edit Mahasiswa" : "Tambah Mahasiswa"}
        </h2>
